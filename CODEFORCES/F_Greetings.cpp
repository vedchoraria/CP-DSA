...
void solve() {
    ...
    vector<pair<int,pair<int,int>>>chuck;
    for(int i =0;i<n;i++){
        int a , b; cin>>a>>b;
        int x = b-a;
        chuck.push_back({x,{a,b}});
    }
    sort(chuck.begin() , chuck.end());
    vector<int>a , b;
    int ans =0;
    a.push_back(chuck[0].second.first);
    b.push_back(chuck[0].second.second);
    for(int i =1;i<n;i++){
        int x = chuck[i].second.first , y = chuck[i].second.second;
